...
| Word / Excel / PDF 内嵌图片 | Embedded Asset Extractor + PaddleOCR | 人工复核 |
...
| 扫描 PDF / 图片 | PaddleOCR / PP-Structure | 人工复核 / 可选 VLM 辅助 |
...
- `PaddleOCR / PP-Structure`：负责扫描页、图片文字、版面结构、表格单元格和坐标级结构识别。
...
| PaddleOCR | 保留并升级 | OCR + 结构化视觉解析能力，承担扫描件、图片文字和部分版面结构 |
