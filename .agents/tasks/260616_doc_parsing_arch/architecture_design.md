...
当前 RustTool 项目以 Rust + Axum + Vue + Tauri 为主体，文档解析能力应作为独立 Parser Worker 接入，而不是把 Python 解释器嵌入 Rust 主进程。

### 9.1 推荐边界

| 层级 | 技术主体 | 职责 |
|------|----------|------|
| 产品与交互 | Vue / Tauri | 上传、任务进度、解析结果展示、人工复核 |
| API 与任务编排 | Rust Axum | 鉴权、文件安全、任务状态、调用 Worker、结果校验 |
| 核心领域模型 | `crates/rust_tool_core` | ParseTask、路由结果、Canonical JSON、质量门禁、复核状态 |
| 文档解析 | Python Parser Worker | MarkItDown、Docling、PaddleOCR、PyMuPDF、pdfplumber、Pandas、DuckDB |
| 交付边界 | Canonical JSON | Rust 只消费经过 schema 校验的结构化结果 |

### 9.2 服务端形态

服务端采用 `Rust Server + Python Worker`：

```mermaid
flowchart LR
    API["Rust Axum Server"] --> Queue["任务队列 / 本地调度"]
    Queue --> Worker["Python Parser Worker"]
    Worker --> Canonical["canonical.json / assets / parquet"]
    API --> Canonical
```

第一版可以由 Rust Server 拉起受控子进程；业务量上来后，Worker 拆成独立进程或容器，并对 OCR、Docling、PaddleOCR-VL 等高成本任务限流。

### 9.3 桌面端形态

桌面端采用 Tauri sidecar：

1. `workers/document_parser_py` 打包成 `document-parser` 可执行文件。
2. 构建脚本按平台生成带 target triple 的 sidecar。
3. `frontend/src-tauri/tauri.conf.json` 通过 `bundle.externalBin` 声明 sidecar。
4. Rust/Tauri 侧通过 JSON 请求 / 响应与 sidecar 通信。
5. 大模型和大体积模型文件不放入基础安装包，优先使用受控缓存、首次下载或企业离线资源包。

### 9.4 不推荐方案

| 方案 | 不推荐原因 |
|------|------------|
| PyO3 嵌入 Python | 依赖隔离、崩溃隔离、跨平台打包和升级都更复杂 |
| 要求用户手动安装 Python | 桌面端交付不可控，环境问题会直接变成产品问题 |
| Rust 重写全部文档解析 | 周期长，无法复用成熟 Office/PDF/OCR/Pandas 生态 |
| 把所有模型打进基础包 | 安装包过大，升级慢，且很多能力第一版并不会高频使用 |
